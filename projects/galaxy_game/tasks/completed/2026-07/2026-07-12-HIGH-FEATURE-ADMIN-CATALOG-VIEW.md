...
- Data path was gitignored outside Rails.root — resolved by using `GalaxyGame::Paths::JSON_DATA` (already defined in project)
...
- **Cross-reference lookup bug**: `File.basename(path, '_bp')` strips `_bp` as a file extension, not string suffix. Fixed by stripping `.json` first then removing `_bp`/`_data` suffixes. (commit `a2377f2d`)
- **Spec comparison operator**: Ruby arrays don't support `<=` — fixed to use `<=>` with `<= 0`. (commit `a2377f2d`)
- **Fused line in controller**: Two edits got fused mid-word (`sortce`). Fixed by replacing the exact corrupted line. (commit `44183dab`)
- **Controller categories hardcoded**: Replaced with dynamic derivation from actual data via `catalog_service.entries.map { |e| e[:category] }.uniq.sort`

### Follow-up tasks needed
- Image generation pipeline setup: Generate PNG files for all 251 blueprints and 193 operational_data entries into `data/json-data/images/` structure
...
- File system globbing with Dir.glob is efficient for 444 files — no need for database indexing at this scale
...
- **Always use existing project path constants** (`GalaxyGame::Paths`) instead of reinventing path resolution
- **Cross-reference lookups must strip extensions before suffixes**: `File.basename(path, '.json').sub(/_bp$/, '')` not `File.basename(path, '_bp')`
- **Ruby array comparison uses `<=>` (spaceship operator), not `<=`** for ordering checks in specs
