...
## Comparable Infrastructure Units (Design Pattern Reference)

For design consistency, review these existing infrastructure units and their integration patterns:

### Planetary Umbilical Hub Mk1 (Small Utility Infrastructure)
```
Category: Infrastructure / Utility
Size: 1.5m × 1.5m × 1.0m (compact)
Mass: 300 kg empty
Build Time: 30 hours
Power: 2 kW
Cost: 25,000 GCC
Tech Gate: advanced_materials
Purpose: Central power/resource transfer hub between base and spacecraft
Hazard Level: 1 (low risk)
Deployment: Surface (no excavation required)
```

**Key Pattern Observation:** Small infrastructure units (utilities) require simple facility types and low-hazard tech gates. Compare to Spin-Gravity Core (8000h, 250kW, 500k GCC, hazard_level 3).

### Workshop Module (Mid-Scale Production Infrastructure)
**Expected specs (reference for mid-size infrastructure):**
- Build time: 2000-4000 hours
- Power: 80-150 kW
- Hazard level: 2 (moderate)
- Underground: Usually yes
- Tech gate: manufacturing or materials_processing

---

## Settlement Infrastructure Tiers (Full Architecture Context)

Current settlement design from existing documentation:

### SURFACE ZONE
```
Tier 1: Power Generation
  - Solar Array (I-beam modular frame, phase_1)
  - RTG Baseload Grid (radioisotope, phase_1, continuous 50-100kW)

Tier 2: Landing & Logistics
  - Craft Garage (airlocks, phase_2)
  - Landing Pad Farm (LOX/fuel, phase_2)

Tier 3: Life Support
  - Utility Hub (scrubbers/compressors/water_recycling, phase_2)
  - Backyard Dome (LEDs + fiber-optic daylight, phase_3, aesthetic/morale)

Connection: Umbilical Hub connects all surface infrastructure to underground via power/resource transfer
```

### SUBSURFACE ZONE (Excavated & Pressurized ~1 atm)

**Phase 2 (Crew Arrival):**
```
Habitat Pods (Earth-normal 21% O₂/78% N₂ atmosphere)
  - Modular 3m × 3m × 2.5m pods
  - Crew capacity: 2-4 per pod
  - Life support: Closed-loop O₂/N₂ recycling via Utility Hub scrubbers

Sleeping Pods (Sleep-only modules)
  - Smaller, pressurized, 1.0g condition critical

Infrastructure Elevator/Lift
  - Personnel access between surface and pod level
  - Power-assisted or mechanical winch backup
```

**Phase 3 (Production & Specialization):**
```
Workshop (3D Printing, Metal Smelting, Manufacturing)
  - Heavy power consumption (150-300 kW)
  - High-temperature operations
  - Connected to ISRU output (regolith, volatiles)

Hydroponics Vault (Food Production)
  - Tier row lights, nutrient recycling
  - Integrated with water_ceiling ("The Gem") for irrigation
  - Support for 10-50 crew depending on depth/scale

SPIN-GRAVITY CORE [THIS UNIT] ← WHERE?
  - Size: 20m × 20m × 12m underground chamber
  - Power: 250 kW sustained
  - Hazard: High (rotating centrifuge, magnetic field)
  - Critical: Long-duration crew health (bone/cardiac conditioning)

Water-Ceiling Atrium ("The Gem")
  - Underground lake/reservoir ceiling serves as:
    - Thermal mass (temperature regulation)
    - Water for hydroponics & life support
    - Radiation shield (regolith + water combined)
    - Psychological benefit (light reflection, aesthetic)
  - Can support 50-100+ crew with sufficient depth

Integration Hub
  - Central routing for power, water, air, waste
  - Connected to all subsurface modules
```

**Phase 4+ (Advanced Infrastructure):**
```
Specialized facilities (smelters, refineries, advanced manufacturing)
  - Mk2/Mk3 equipment variants
  - Scaled-up production capacity
```

### Critical Assumptions in Current Design
1. **Excavation phasing**: Surface → Phase 2 (basic subsurface) → Phase 3 (full subsurface)
2. **Power hierarchy**: RTG baseload + solar + on-demand production
3. **Crew phases**: Scout → Family → Industrial workforce (50-100+ crew)
4. **Atmosphere**: Separate surface (vacuum) / subsurface (1 atm Earth-normal)

---

## Tech Tree Context

### Gravitational Engineering (particle_physics.json tier_1b)

**STATUS**: Exists in tech tree but tech_tree entry details are incomplete/unclear in current system.

**Known Reference**: Blueprint requires `"required_technology": "gravitational_engineering"`

**Questions to clarify in synthesis:**
- What techs unlock gravitational_engineering? (prerequisites)
- What techs does gravitational_engineering unlock? (unlocks)
- What is tech cost in GCC and research points?
- Is it gated behind other physics techs (e.g., electromagnetism)?

**Related techs (likely):**
- magnetic_systems (foundation for mag-lev bearings)
- advanced_materials (carbon fiber, composites)
- electrical_engineering (power systems)
- precision_manufacturing (alignment requirements)

---

## ISRU Equipment Chain Context (From 2026-08-01 Validation)

The Spin-Gravity Core operates independently but exists in ecosystem with:

```
Regolith Harvesting Rover (Mobile craft, surface scraper)
    ↓ (infinite regolith feed)
Thermal Extraction Unit Mk1 (Surface/subsurface, low-mid temp baking)
    ↓ (drives off H₂O, CO₂, N₂)
Planetary Volatiles Extractor Mk1/2/3 (High-temp oxide reduction)
    ↓ (production)
    ├→ Elemental O₂ (Tank Farm LOX supply)
    └→ Depleted regolith (Workshop inputs)
```

**Integration question for synthesis**: Does Spin-Gravity Core require O₂ from this chain, or does it tap surface power grid only?

---

## Known Gaps & TBD Items (Work in Progress)

### Tech Tree Incomplete
Four volatile extraction techs referenced in ISRU equipment do NOT yet exist in tech_tree:
- `thermal_volatile_extraction_tech` (TEU Mk1)
- `planetary_volatiles_extraction_tech` (PVE Mk1)
- `advanced_volatiles_processing_tech` (PVE Mk2)
- `extreme_environment_volatile_extraction_tech` (PVE Mk3)

**Status**: Placeholders created; real tech_tree entries needed in mining_resource_processing.json

### Stub Units Identified (Require Design Review)
- AeroFab CNC Module Mk1 (production/fabricators)
- Volatile Systems Integrator Mk1 (production/refineries)
- Both have 1h/1 GCC placeholder operational_data

**Status**: Awaiting decision — real units or intentional stubs?
