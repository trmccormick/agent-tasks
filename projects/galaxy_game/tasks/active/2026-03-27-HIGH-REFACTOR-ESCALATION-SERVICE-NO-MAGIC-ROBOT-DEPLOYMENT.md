...
Do this before reading any code, before synthesis, before anything else.
...
> ❌ If git mv fails with "not tracked": use plain mv then git add at the final path.
> ❌ Do NOT proceed if two results appear — remove the stale copy first.
